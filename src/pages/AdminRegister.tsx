...
export default function AdminRegister() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await register({ email, password, shop_name: shopName })
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <h1 className="text-3xl font-heading text-text-primary text-center mb-2">Create Your Shop</h1>
        <p className="text-text-secondary text-sm text-center mb-8">Set up your barbershop in minutes</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-text-secondary text-sm mb-1 block">Shop Name</label>
            <input type="text" value={shopName} onChange={(e) => setShopName(e.target.value)} className={inputClass} placeholder="The Gentleman's Cut" required />
          </div>
          <div>
          ...
          <button type="submit" disabled={loading} className="w-full bg-gold hover:bg-gold-hover text-bg-primary py-3 text-sm font-semibold transition-colors disabled:opacity-50 hover:shadow-[0_0_20px_rgba(200,165,90,0.3)]">
            {loading ? 'Creating...' : 'Create Your Shop'}
          </button>
        </form>
        <p className="text-text-secondary text-sm text-center mt-6">
          Already have a shop?{' '}
          <Link to="/admin/login" className="text-gold hover:text-gold-hover transition-colors">Sign in</Link>
        </p>
      </div>
    </div>
  )
